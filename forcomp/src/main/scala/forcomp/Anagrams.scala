...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (for ((chr, l) <- w.toLowerCase groupBy (c => c)) yield (chr, l.length())).toList.sorted
    ...
    def comb(occurrences: Occurrences): List[Occurrences] = {
      if (occurrences.isEmpty) List(List())
        ...
        (for {
          ...
          rest <- comb(occurrences.tail)
        } yield (occurrences.head._1, n) :: rest).toList
    }
    val subsets = occurrences.toSet[(Char, Int)].subsets.map(_.toList).toList
    (for {
      s <- subsets
    } yield comb(s)).flatten
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (if (y.isEmpty) x
      ...
      val k = y.head._1
      val v = y.head._2
      if (xMap.contains(k)) subtract(xMap.updated(k, xMap.apply(k) - v).toList, y.tail)
      else subtract(xMap.toList, y.tail)
    }).filter(c => c._2 > 0)
  }
